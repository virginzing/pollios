...
RSpec.describe Campaign, :type => :model do
  ...
  describe "#prediction" do
    ...
    it "can receive reward" do
      ...
      expect(CampaignMember.where(member_id: member.id, campaign_id: campaign.id, luck: true).count).to eq(1)
    end

    it "don't receive reward when expire" do
      campaign.update(expire: 1.days.ago)
      campaign.reload.prediction(member.id, poll.id)
      expect(CampaignMember.where(member_id: member.id, campaign_id: campaign.id, luck: true).count).to eq(0)
    end
