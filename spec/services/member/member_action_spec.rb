...
RSpec.describe "[Service: #{pathname.dirname.basename}/#{pathname.basename}]\n\n Member::MemberAction" do
  ...
  context '#add_friend: Member[1] sends add friend request to member[2]' do
    ...
    it "- Member[1]'s outgoing friend request to member[2] is created." do
      expect(Member::MerrmberList.new(@member_1).already_sent_request_to?(@member_2)).to be true
    end
  ...
  context '#unfriend: Member[1] unfriends member[2]' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
      Member::MemberAction.new(@member_1, @member_2).add_friend
      ...
      Member::MemberAction.new(@member_1, @member_2).unfriend
    end

    it '- Member[2] disappears from incoming unfriend request list of member[1]' do
      expect(Member::MemberList.new(@member_1).not_exist_incoming_request?(@member_2)).to be true
    end

    it '- Member[1] disappears from outgoing unfriend request list of member[2]' do
      expect(Member::MemberList.new(@member_2).not_exist_outgoing_request?(@member_1)).to be true
    end
  end

  context '#unfriend: Member[1] can not unfriend member[2]' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
    end
    ...
    it '- A member[1] can not unfriend your self' do
      ...
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.unfriend_self)
    end
    ...
    it '- A member[1] are not friend member[2]' do
      ...
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.not_friend(@member_2))
    end
  end

  context '#follow: Member follows a celebrity' do
    before(:context) do
      @member = create(:member)
      @celebrity_member = create(:celebrity_member)

      Member::MemberAction.new(@member, @celebrity_member).follow
    end

    it "- The member's following list contains the celebrity" do
      expect(Member::MemberList.new(@member).followings).to include(@celebrity_member)
    end

    it "- The celebrity's follower list contains the member" do
      expect(Member::MemberList.new(@celebrity_member).followers).to include(@member)
    end
  end

  context '#follow: A member can not follow a celebrity when' do
    before(:context) do
      @member = create(:member)
    ...
    it '- The celebrity can not follow self' do
      expect { Member::MemberAction.new(@celebrity_member, @celebrity_member).follow } \
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.follow_self)
    end

    it '- The member has already followed the celebrity' do
      Member::MemberAction.new(@member, @celebrity_member).follow

      expect { Member::MemberAction.new(@member, @celebrity_member).follow } \
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.already_followed)
    end

    it '- The member who you want to follow is not celebrity' do
      expect { Member::MemberAction.new(@member, @another_member).follow  } \
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.not_official_account)
    end

    it '- the member has already blocked the celebrity' do
      Member::MemberAction.new(@member, @celebrity_member).block

      expect { Member::MemberAction.new(@member, @celebrity_member).follow  } \
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.already_blocked(@celebrity_member))
    end

    it '- The member is blocked by the celebrity' do
      Member::MemberAction.new(@celebrity_member, @member).block

      expect { Member::MemberAction.new(@member, @celebrity_member).follow  } \
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.blocked_by(@celebrity_member))
    end
  end

  context '#unfollow: A Member unfollows a celebrity' do
    before(:context) do
      @member = create(:member)
      @celebrity_member = create(:celebrity_member)

      Member::MemberAction.new(@member, @celebrity_member).follow
      Member::MemberAction.new(@member, @celebrity_member).unfollow
    end

    it '- A celebrity_member disappears from followings list of member[1]' do
      expect(Member::MemberList.new(@member).already_follow_with?(@celebrity_member)).to be false
    end

    it '- A member[1] disappears from follower list of celebrity_member' do
      expect(Member::MemberList.new(@celebrity_member).followers).not_to include(@member)
    end
  end

  context '#unfollow: A member can not unfollow a celebrity when' do
    before(:context) do
      @member = create(:member)
      @celebrity_member = create(:celebrity_member)
    end
    ...
    it '- A member[1] can not send unfollow themself' do
      ...
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.unfollow_self)
    end
    ...
    it '- A member[1] are not following member[2]' do
      ...
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.not_following)
    end
  end

  context '#deny: Member[2] denies friend request from member[1]' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
      Member::MemberAction.new(@member_1, @member_2).add_friend

      Member::MemberAction.new(@member_2, @member_1).deny_friend_request
    end

    it "- Member[2] does not exist in Member[1]'s outgoing friend request list." do
      expect(Member::MemberList.new(@member_1).not_exist_outgoing_request?(@member_2)).to be true
    end

    it "- Member[1] does not exist in Member[2]'s incoming friend request list." do
      expect(Member::MemberList.new(@member_2).not_exist_incoming_request?(@member_1)).to be true
    end
  end

  context '#deny: Member[2] can not deny friend request from member[1] when' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
    end
  ...
  context '#cancel: A member[1] cancels friend request from member[1]' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
      Member::MemberAction.new(@member_1, @member_2).add_friend

      Member::MemberAction.new(@member_1, @member_2).cancel_friend_request
    end
  ...
  context '#cancel: A member[1] can not cancel friend request to member[2]' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
    end

    it '- A member[1] dose not send add friend request to member[2]' do
      expect { Member::MemberAction.new(@member_2, @member_1).cancel_friend_request } \
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.not_exist_outgoing_request)
    end
  end
  ...
  context '#block: A member[1] blocks member[2]' do
    ...
    it "- Member[1]'s block list contains member[2]." do
      expect(Member::MemberList.new(@member_1).already_block_with?(@member_2)).to be true
    end
  end

  context '#block: Member[1] can not block member[2] when' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
    end

    it '- Member[1] can not block self' do
      expect { Member::MemberAction.new(@member_1, @member_1).block } \
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.block_self)
    end

    it '- Member[1] has already blocked member[2]' do
      Member::MemberAction.new(@member_1, @member_2).block
  ...
  context '#unblock: Member[1] unblocks Member[2]' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)

      Member::MemberAction.new(@member_1, @member_2).block
      Member::MemberAction.new(@member_1, @member_2).unblock
    end
  ...
  context '#unblock: Member[1] can not unblock member[2] when' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
    end

    it '- Member[1] unblock self' do
      expect { Member::MemberAction.new(@member_1, @member_1).unblock } \
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.unblock_self)
    end

    it '- Member[1] are not blocking member[2]' do
      Member::MemberAction.new(@member_1, @member_2).block
      Member::MemberAction.new(@member_1, @member_2).unblock
      ...
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.not_blocking(@member_2))
    end
  end

  context '#report: Member[1] reports member[2]' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
    end
  ...
  context '#report: Member[1] can not report member[2] when' do
    before(:context) do
      @member_1 = create(:member)
      @member_2 = create(:member)
    end

    it '- Member[1] report self' do
      expect { Member::MemberAction.new(@member_1, @member_1).report(true) } \
      .to raise_error(
        ExceptionHandler::UnprocessableEntity,
        GuardMessage::Member.report_self)
    end
  end
end