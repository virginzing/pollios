...
RSpec.describe "[Service: #{pathname.dirname.basename}/#{pathname.basename}]\n\n Member::GroupActionLegacyLegacy" do
    ...
    context "#create: A member create group that needs approve" do
        let(:new_group) { Member::GroupActionLegacy.new(group_admin).create(FactoryGirl.attributes_for(:group_that_need_approve)) }
    ...
    context "#create: A member create group that doesn't need approve" do
        let(:new_group) { Member::GroupActionLegacy.new(group_admin).create(FactoryGirl.attributes_for(:group_that_dont_need_approve)) }
    ...
    context "#create: A member create group with cover url" do
        let(:new_group) { Member::GroupActionLegacy.new(group_admin).create(FactoryGirl.attributes_for(:group_with_cover_url)) }
    ...
    context "#create: A Member who is a Company create a group" do
        ...
        let(:new_group) { Member::GroupActionLegacy.new(company).create(FactoryGirl.attributes_for(:group)) }
    ...
    context "#create #invite: A member create group with invitation list: 103,104,105,107,108,109" do
        let(:new_group) { Member::GroupActionLegacy.new(group_admin).create(FactoryGirl.attributes_for(:group_with_invitation_list)) }
    ...
    context "#join: A member request joining group that doesn't need approval" do
        let!(:group) { Member::GroupActionLegacy.new(group_admin).create(FactoryGirl.attributes_for(:group_that_dont_need_approve)) }
        let!(:group_action) { Member::GroupActionLegacy.new(group_admin, group) }
    ...
    context "#join: A member request joining group that needs approval" do
        let(:group) { Member::GroupActionLegacy.new(group_admin).create(FactoryGirl.attributes_for(:group_that_need_approve)) }
    ...
    context "#invite: Admin member invite user to group" do
        let(:group) { Member::GroupActionLegacy.new(group_admin).create(FactoryGirl.attributes_for(:group)) }
