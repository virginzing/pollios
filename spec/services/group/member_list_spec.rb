...
RSpec.describe "[Service: #{pathname.dirname.basename}/#{pathname.basename}]\n\n Group::MemberList" do
  ...
  let(:group_member_listing) { Group::MemberList.new(new_group) }
  ...
  context 'Including' do
    ...
    it '- A member is include group admin or group member ' do
      ...
      expect(group_member_listing.member_or_admin?(group_admin)).to be true
      expect(group_member_listing.member_or_admin?(new_member1)).to be true
    end

    it '- A member is include group admin' do
      expect(group_member_listing.admin?(group_admin)).to be true
    end

    it '- A member is include group member' do
      new_group.group_members.create(member_id: new_member1.id, is_master: false, active: true)

      expect(group_member_listing.member?(new_member1)).to be true
    end
    ...
    it '- A member is include pending member' do
      ...
      expect(group_member_listing.pending?(new_member1)).to be true
    end

    it '- A member is include requesting member' do
      new_group.request_groups.create(member_id: new_member1.id)
      
      expect(group_member_listing.requesting?(new_member1)).to be true
    end

    it "- A member isn't include group" do
      expect(group_member_listing.admin?(new_member1)).to be false
      expect(group_member_listing.member?(new_member1)).to be false
      expect(group_member_listing.pending?(new_member1)).to be false
      expect(group_member_listing.requesting?(new_member1)).to be false
    end
  end
  ...
  context 'Listing' do
    ...
    it '- Admins of group' do
      ...
      expect(group_member_listing.admins).to match_array([group_admin, new_member1, new_member2, new_member3])
    end
    ...
    it '- Members of group' do
      ...
      expect(group_member_listing.members).to match_array([new_member1, new_member2, new_member3, new_member4])
    end
    ...
    it '- Pending members of group' do
      ...
      expect(group_member_listing.pending).to match_array([new_member1, new_member2, new_member3, new_member4])
    end
    ...
    it '- Requesting members of group' do
      ...
      expect(group_member_listing.requesting).to match_array([new_member1, new_member2, new_member3, new_member4])
    end
    ...
    it '- Members join requestly' do
      ...
      expect(group_member_listing.join_recently)
  ...
  context 'Filtering' do
    ...
    it '- Member ids in group from list of member id' do
      ...
      expect(group_member_listing.filter_members_from_list([new_member1.id, new_member2.id, new_member3.id]))
    ...
    it '- Member ids not in group from list of member id' do
      ...
      expect(group_member_listing.filter_non_members_from_list([new_member1.id, new_member2.id, new_member3.id]))
