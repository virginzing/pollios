class PollMember < ActiveRecord::Base
  ...
  def self.find_poll_celebrity(type)
    celebrtiy = Member.having_member_type(:celebrity).pluck(:id)
    find_poll_member = where("member_id IN (?) AND share_poll_of_id = 0", celebrtiy)
  ...
  def self.find_poll_shared(friend_id, type)
    find_poll_shared = where("member_id IN (?) AND share_poll_of_id != 0",friend_id)
  ...
  def self.timeline(member_id, friend_id, type)
    ...
    list_ids = ids + find_poll_celebrity(type)
