class NotifyExpireSubscriptionWorker
  ...
  def perform(sender_id, list_member, custom_data = {})
    ...
    find_recipient.each do |member|
      hash_custom = {
        ...
        action: ACTION[:subscription_expire],
        worker: WORKER[:notify_expire_subscription]
      }
    ...
    puts "NotifyExpireSubscriptionWorker => #{e.message}"
  end
