class V6::FriendPollInProfile
  ...
  def poll_created
    ...
    query = Poll.without_deleted.available.joins(:poll_members).includes(:choices, :member, :poll_series, :campaign, :poll_groups).
  ...
  def poll_voted
    query = Poll.without_deleted.available.joins(:history_votes).includes(:choices, :member, :campaign, :poll_groups)
  ...
  def poll_watched
    query = Poll.without_deleted.available.joins(:watcheds).includes(:choices, :member, :poll_series, :campaign, :poll_groups)
  ...
  def poll_voted_with_visibility(next_cursor = nil, limit_poll = LIMIT_POLL)
    query = Poll.without_deleted.without_my_poll(friend_id).load_more(next_cursor).available.joins(:history_votes).includes(:choices, :member, :poll_series, :campaign, :poll_groups)
