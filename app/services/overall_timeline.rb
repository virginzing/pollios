class OverallTimeline
  ...
  def unvote_count
    poll_id_from_poll_member = PollMember.available(hide_poll_ids).where("id IN (?)", cached_poll_ids_of_poll_member).map(&:poll_id)
  ...
  def find_poll_me_and_friend_and_group_and_public
    ...
    query = PollMember.available(hide_poll_ids).joins(:poll).where("(#{poll_member_query} AND #{poll_unexpire})" \
  ...
  def find_poll_share
    ...
    query = PollMember.available(hide_poll_ids).joins(:poll).where("(#{query_poll_shared} AND #{poll_unexpire}) " \
