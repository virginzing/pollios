class MembersController < ApplicationController
  ...
  def notify
    @notifications = @current_member.received_notifies.without_deleted.where('created_at >= ?', Time.now - 10.days).order('created_at DESC').limits(200).paginate(page: params[:next_cursor])
